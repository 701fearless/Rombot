...
比赛演示在两者之间增加了一条完全缓存化的空间链路：

```text
上传 room1–7 原图（浏览器 SHA-256 匹配）
  → /feed?sceneId=roomN
  → 暂停识别只开放带 prebuiltGlbUrl 的 Tag
  → /space?sceneId=roomN&frameId=...&objectId=...
  → 同一 Three.js Scene 加载户型 GLB + 缓存家具 GLB
```
...
| Space / 户型组合页 | `/space` | 匹配预处理户型，并组合预览户型与缓存家具 GLB |
...
- `room1–7` 使用原图 SHA-256 匹配预设；placeholder 阶段统一复用 `sample_data/floorplans/whitebox.glb`。
- `/api/feed/prebuilt-asset` 只读取视频 2 已生成模型，不触发外部 Provider。
- Space 在同一 Three.js 场景加载户型和家具，支持 X/Z、旋转、缩放与重置。
- Space Viewer 支持自动墙体剖面：中央 3×3 射线选择最遮挡的一面墙，在总计 150° 的正对范围内平滑淡隐，俯视时自动恢复。
- Docker 多阶段构建，部署镜像不安装 Torch/Transformers。

### 2.2 已有但仍是过渡实现

- `/space` 的比赛路径已接预处理户型和缓存家具；Ark 实时重建接口仍保留给后续非比赛流程。
- `/api/feed/select-object` 可走真实生图/生 3D，但同步等待任务完成。
- 分割默认可使用 bbox mock；接入 SAM3 后才是精细 mask。
- `/api/room/scan` 返回 mock 房间，不是真实户型扫描。
- `/api/floorplan/reconstruct` 是真实的户型图结构识别与白模生成，两者不能混用。
...
│  │  │  ├─ SpacePlaceholder.tsx    # Space 参数、户型上传与结果页面
│  │  │  ├─ FloorplanViewer.tsx     # Three.js 白模交互预览
...
│  │  │  ├─ model3d/                # 3D Provider
│  │  │  ├─ room_scan/              # mock 场景
│  │  │  └─ floorplan_whitebox/      # Ark 户型解析、Schema、GLB Builder
...
1. 解析并保留 Feed 查询参数。
2. 读取本地 JPEG、PNG 或 WebP 户型图并在浏览器计算 SHA-256。
3. 调用 `GET /api/floorplan/presets` 匹配 `room1–7`，不上传图片、不调用 Ark。
4. 携带 `sceneId` 进入 Feed，并只允许点击带 `prebuiltGlbUrl` 的家具 Tag。
5. 用 `frameId + objectId` 调 `GET /api/feed/prebuilt-asset`。
6. 用同一个 Three.js Scene 加载户型和家具 GLB，按 `estimatedDimensions` 缩放、居中落地。
7. 提供 X/Z 位移、左右旋转、缩放和重置控件。

比赛主链明确不调用 `POST /api/floorplan/reconstruct` 或
`POST /api/feed/select-object`。以后需要开放非缓存家具时，才把同步 `select-object`
改为异步任务；不要让 Feed 或 Space 在交互热路径等待数分钟。
...
运行时以 `/openapi.json` 为唯一机器可读事实，`/dashboard` 会动态读取它。当前业务路由共 19 个：

| 方法 | 路径 | 请求 | 响应 | 用途 |
|---|---|---|---|---|
| GET | `/health` | 无 | 状态 JSON | 健康检查 |
| POST | `/api/feed/detect` | `DetectRequest` | `DetectResponse` | 暂停帧匹配或实时识别 |
| GET | `/api/feed/prebuilt-asset` | `frameId + objectId` Query | `PrebuiltAssetResponse` | 读取缓存家具 GLB，不触发生成 |
...
| POST | `/api/floorplan/reconstruct` | `FloorplanReconstructRequest` | `FloorplanReconstructResponse` | 户型图识别并生成白模 |
| POST | `/api/floorplan/build-whitebox` | `FloorplanWhiteboxScene` | `FloorplanBuildWhiteboxResponse` | 结构 JSON 离线生成白模 |
| GET | `/api/floorplan/presets` | 无 | `FloorplanPresetsResponse` | room1–7 SHA-256 与预处理资源 |
| GET | `/api/floorplan/presets/{scene_id}` | Path | `FloorplanPreset` | 恢复单个预处理户型 |
...
后端也允许读取 `/sample_data/floorplans/*` 或 `/outputs/*` 下的站内测试图片，但拒绝任意绝对路径。成功响应：

```json
{
  "sceneId": "floorplan_a1b2c3d4e5",
  "status": "succeeded",
  "sceneUrl": "/outputs/floorplans/.../normalized_scene.json",
  "whiteboxGlbUrl": "/outputs/floorplans/.../whitebox.glb",
  "aiRawUrl": "/outputs/floorplans/.../ai_raw.json",
  "originalImageUrl": "/outputs/floorplans/.../original.png",
  "warnings": []
}
```

`knownLength` 当前只记录在 `ai_raw.json`，不会校准坐标；传入后响应 warnings 会明确提示。固定建模单位为米，墙高 `3.0m`、墙厚 `0.1m`。
...
```text
sceneId → 预处理户型 GLB
frameId/objectId → 缓存家具 GLB
两者 → 同一 Three.js Scene → 基础变换控件
```

下一版空间编辑器应在当前 Viewer 之上增加拖拽、精确碰撞和保存，不需要重新实现资产解析。
Feed 继续遵守带 `sceneId + frameId + objectId` 的 URL 协议。
...
- 校验跳转参数。
- 加异步生成任务 API。
- 在现有户型白模 Viewer 中加载家具 GLB、基础摆放和错误恢复。
- 生成结果缓存。
