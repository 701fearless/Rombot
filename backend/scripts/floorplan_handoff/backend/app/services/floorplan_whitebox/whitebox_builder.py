...
def build_whitebox_primitives(scene: FloorplanWhiteboxScene) -> list[Primitive]:
    ...
    primitives.append(build_floor(scene.floorPolygon))

    for wall in scene.walls:
        primitives.extend(build_wall_blocks(wall, fixtures_by_wall.get(wall.id, [])))
    ...
    for fixture in scene.wallFixtures:
        ...
        if fixture.type == "door":
            primitives.extend(build_door(wall, fixture))
        elif fixture.type == "window":
            primitives.extend(build_window(wall, fixture))
