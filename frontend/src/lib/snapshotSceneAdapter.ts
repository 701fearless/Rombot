...
export function snapshotToSceneResponse(snapshot: SceneSnapshot): Record<string, unknown> {
  ...
  return { sceneId: snapshot.sceneId, unit: snapshot.unit, room: { width: maxX - minX, depth: maxZ - minZ, height: Math.max(2.4, ...snapshot.room.walls.map((wall) => numberValue(wall.height, 0))) }, objects: snapshot.objects.map((item) => ({ id: item.instanceId, label: item.semantic.label, name: item.semantic.name, position: item.transform.position, rotation: item.transform.rotation, size: item.geometry.size.map((value, index) => value * item.transform.scale[index]), glbUrl: item.geometry.glbUrl ?? null })), openings: [], suggestions: [] }
}
